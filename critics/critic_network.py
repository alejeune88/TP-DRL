...
class CriticNetwork(GenericNet):
    ...
    def compute_validation_loss(self, validation_loader, train=False):
        ...
        for step, (batch_s, batch_a, batch_t) in enumerate(validation_loader):
    ...
    def update_mc(self, params, dataset, train=False, save_best=True):
        """
        ...
        """
        best_loss = 10000
        loader = data.DataLoader(dataset=dataset, batch_size=params.batch_size, shuffle=params.shuffle, num_workers=params.nb_workers, )
        for epoch in range(params.nb_batches):
    ...
    def compute_valid_td(self, params, dataset):
        ...
        """
        train_sampler, valid_sampler = make_subsets_samplers(dataset)
        t_loader = data.DataLoader(dataset=dataset, batch_size=params.batch_size, num_workers=params.nb_workers, sampler=train_sampler)
        v_loader = data.DataLoader(dataset=dataset, batch_size=params.batch_size, num_workers=params.nb_workers, sampler=valid_sampler)
